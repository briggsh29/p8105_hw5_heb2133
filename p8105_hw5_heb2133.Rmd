...
for (i in 1:6) {
    ...
    output[[i]] = rerun(5000, hypoth_test(mu = mu_var_list[[i]])) %>% 
    bind_rows() %>% 
    select(estimate, p.value)
}
...
Plot showing the proportion of times the null was rejected (y axis) and the true value of μ (x axis). Describe the association between effect size and power.
