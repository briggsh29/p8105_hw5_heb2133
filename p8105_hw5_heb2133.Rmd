...
l_study_df = 
  ...
  select(subject_id, study_arm, week_number, observations, file_name)
```

Make a spaghetti plot showing observations on each subject over time. Comment on differences between groups.

```{r}
l_study_df %>% 
  ggplot(aes(x = week_number, y = observations, color = study_arm)) + 
  geom_point() +
...
Looking at this plot, we can see that between the control and experimental arms - the control arm appears to diverge from the experiemental arm in terms of observations. 
